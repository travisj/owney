...
use ms_authn::{AuthInput, Authenticator};
use ms_events::EventBus;
use ms_smtp_in::{DeliverError, InboundMail, MailHandler, RcptVerdict};
...
impl MailHandler for ServerCore {
    ...
    async fn deliver(&self, mail: InboundMail) -> Result<(), DeliverError> {
        ...
        for recipient in &mail.recipients {
            ...
            let pgp = ms_pgp::pipeline::inbound(&self.storage, account.id, raw.clone())
                .await
                .map_err(|err| DeliverError(format!("pgp: {err}")))?;
            for address in &pgp.key_changes {
                self.events.publish(ms_events::Event::Security {
                    account_id: Some(account.id),
                    kind: ms_events::SecurityEventKind::Other,
