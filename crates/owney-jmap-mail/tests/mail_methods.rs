...
use owney_jmap_mail::MAIL_CAPABILITY;
...
impl owney_delivery::Submitter for MockSubmitter {
    fn submit(
        &self,
        ...
        Box<dyn Future<Output = Result<Vec<uuid::Uuid>, owney_delivery::SubmitError>> + Send + '_>,
    > {
        self.submitted
            .lock()
            .expect("lock")
...
impl Harness {
    async fn call(&self, calls: Value) -> Vec<jmap_core::Invocation> {
        let request = serde_json::from_value(json!({
            "using": [CORE_CAPABILITY, MAIL_CAPABILITY, owney_jmap_mail::SUBMISSION_CAPABILITY],
            "methodCalls": calls,
        }))
        .expect("request");
        self.dispatcher
