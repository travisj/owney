...
use ms_authn::cache::DnsCaches;
use ms_authn::{AuthInput, Authenticator};
use ms_core::AccountId;
use ms_delivery::{DeliveryParams, DeliveryService, DkimKeys, Relay, StaticRouter};
...
async fn start_receiver() -> Receiver {
    ...
    tokio::spawn(ms_smtp_in::server::run_listener(listener, params, handler));
...
fn test_message() -> Vec<u8> {
    format!(
        ...
        ms_core::time::rfc2822_utc(1_783_137_275),
    )
    .into_bytes()
}
