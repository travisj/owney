...
use ms_core::{AccountId, EmailId, MailboxId};
...
impl McpCtx {
    ...
    pub async fn search(
        ...
        Ok(json!({
            ...
            "results": rows.iter().map(|r| json!({
                ...
                "subject": r.subject,
                "keywords": r.keywords,
                "receivedAt": ms_core::time::iso8601_utc(r.received_at),
            })).collect::<Vec<_>>(),
        }))
    }
    ...
    pub async fn create_draft(
        ...
        let ingested = self
            ...
            .ingest_email_into(
                self.account_id,
                raw,
                ms_storage::MailboxTarget::Id(drafts),
                None,
            )
            .await?;
        self.storage
        ...
        Ok(json!(
            actions
                ...
                .map(|a| json!({
                    ...
                    "createdAt": ms_core::time::iso8601_utc(a.created_at),
                }))
                .collect::<Vec<_>>()
        ))
    }
    ...
    pub async fn undo_action(&self, action_id: &str) -> Result<Value, ServiceError> {
        ...
        ms_ai::undo_action(&self.storage, self.account_id, id)
            .await
