...
use owney_events::{DeliveryStatus, Event, EventBus};
...
fn build_dsn(params: &DeliveryParams, item: &QueueItem, error: &str, status_code: &str) -> Vec<u8> {
    ...
    let date = owney_core::time::rfc2822_utc(unix_now());
...
mod dsn_tests {
    ...
    fn minimal_item() -> QueueItem {
        QueueItem {
            id: uuid::Uuid::now_v7(),
            account_id: owney_core::AccountId::new(),
            blob_id: owney_core::BlobId([0u8; 32]),
